...
log close _all
log using "$logDir/UNCTAD_import.smcl", replace
...
foreach f in `fileList' {
    ...
    ** loop over first line of each each to find country name
    local ctyName = ""
    foreach sheetName in inflows outflows instock outstock {
        import excel using "source_data/UNCTAD/webdiaeia2014d3_`ctyCode'.xls", ///
            sheet("`sheetName'") clear allstring
        local ctyName = A[1]
        if ~missing("`ctyName'") {
            continue, break
        }
    ...
    if missing("`ctyName'") {
        disp as error "All 4 sheets do not contain country Name in the first row for `ctyCode'. Check"
        error 1
    }
